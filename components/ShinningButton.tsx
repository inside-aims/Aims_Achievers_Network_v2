...
export default function ShiningButton({ label, className, ...props }: ShiningButtonProps) {
  return (
    <button
      className={cn(
        "group cursor-pointer rounded-xl border-4 border-violet-800 border-opacity-0 bg-transparent p-1 transition-all duration-500 hover:border-opacity-100",
        className
      )}
      {...props}
    >
