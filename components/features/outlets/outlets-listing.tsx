...
import { OutletCardSkeleton } from "@/components/ui/skeleton";

const stats: { label: string; value: string; icon: LucideIcon; color: string }[] = [
  { icon: Award,     value: "50+",    label: "Master Craftsmen",  color: "primary"           },
  { icon: Users,     value: "8,500+", label: "Satisfied Client",  color: "secondary"         },
  { icon: TrendingUp,value: "15K",    label: "Award Delivered",   color: "accent-foreground" },
  { icon: Heart,     value: "98%",    label: "Client Satisfaction",color: "muted-foreground" },
];
...
export default function OutletsListing() {
    ...
    const interval = setInterval(() => {
      setActiveImages((prev) => {
        ...
        outlets.forEach((outlet) => {
          next[outlet.id] = ((prev[outlet.id] || 0) + 1) % outlet.portfolioImages.length;
        });
        return next;
      });
    }, 3000);
  ...
  const featuredVendors = outlets.filter(
    (vendor) =>
      ...
      <div className="relative z-10 space-y-8">
        <div className="w-full flex flex-col gap-2 md:gap-4">
          <div className="parent-header">
            <h3 className="feature-header leading-tight">Where Legends Are Crafted</h3>
            <p className="feature-subheader max-w-4xl leading-relaxed">
              {`Connect with master artisans who've created over`}
              <span className="text-secondary font-bold"> 15,000 masterpieces</span> for champions
              across Africa
            </p>
          </div>
