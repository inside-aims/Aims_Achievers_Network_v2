...
const Gallery = () => {
  ...
  const { photos, events, categories, universities, loading, error } = useGallery();
  ...
  const filteredPhotos = photos.filter((photo) => {
    ...
    const matchesUniversity = selectedUniversity === "All Universities" || photo.university === selectedUniversity;
    const matchesSearch =
  ...
  return (
    <div className="min-h-screen relative">
      ...
      {loading ? (
        ...
        <div className="feature text-center text-destructive">{error}</div>
      ) : (
        <>
          <GalleryStats
            total={photos.length}
          ...
          <div className="feature">
            ...
            {filteredPhotos.length === 0 && (
              <div className="text-center">
                <Camera className="w-20 h-20 mx-auto text-muted-foreground/30 mb-6" />
