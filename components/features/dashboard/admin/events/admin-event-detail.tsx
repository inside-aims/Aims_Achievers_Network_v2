...
import { getEventTicketInfo, getEventScanCodes } from "@/components/features/tickets/mock-data";
...
export function AdminEventDetail({ base, eventId }: Props) {
  ...
  const ticketStats = useMemo(() => {
    ...
    const revenuePesewas = info.ticketTypes.reduce((s, t) => s + t.quantitySold * t.pricePesewas, 0);
  ...
  const hasTicketing       = ticketingEnabled && !!ticketEventId;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {showTicketStats ? (
          <>
            <StatCard label="Tickets Sold"   value={ticketStats.sold}    sub="across all types"   icon={Tag}      />
            <StatCard label="Ticket Revenue" value={ticketStats.revenue} sub="gross revenue"       icon={Coins}    variant="success" />
            <StatCard label="Ticket Types"   value={ticketStats.types}   sub="available types"     icon={Tag}      variant="info" />
            <StatCard label="Total Scans"    value={ticketStats.scans}   sub="gate scan events"    icon={BarChart3} variant="warning" />
          </>
        ) : (
          <>
            <StatCard label="Total Votes"  value={stats.totalVotes}      sub={stats.priceLabel}          icon={BarChart3} />
            <StatCard label="Revenue"      value={stats.revenue}         sub="gross revenue"             icon={Coins}     variant="success" />
            <StatCard label="Categories"   value={stats.totalCategories} sub="award categories"          icon={Tag}       variant="info" />
            <StatCard label="Nominees"     value={stats.totalNominees}   sub="across all categories"     icon={Users}     variant="warning" />
          </>
        )}
      </div>
      ...
      {hasTicketing ? (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="tickets" className="mt-4">
            <EventTicketsSection eventId={ticketEventId!} readonly />
          </TabsContent>
        </Tabs>
      ) : (
        event.categories.length > 0 && (
