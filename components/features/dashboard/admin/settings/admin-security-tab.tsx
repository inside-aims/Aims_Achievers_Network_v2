...
import { ADMIN_PASSWORD_FIELDS, type AdminPwFieldId } from "./admin-settings.data"
import { Field, SaveBar, SectionCard, PasswordInput } from "@/components/features/dashboard/user/settings/settings-primitives"

export function AdminSecurityTab() {
  const [passwords, setPasswords] = useState<Record<AdminPwFieldId, string>>({
    "cur-pw":  "",
    "new-pw":  "",
    "conf-pw": "",
  })
  ...
  function setField(id: AdminPwFieldId, val: string) {
    ...
    <div className="space-y-5">
      <SectionCard title="Change password" desc="Use a strong password you don't use on other sites.">
        <div className="space-y-4 max-w-sm">
          {ADMIN_PASSWORD_FIELDS.map(({ id, label, placeholder }) => (
        ...
        <SaveBar onSave={savePassword} saved={saved} disabled={!canSave} />
      </SectionCard>
        ...
        <div className="flex items-center gap-2.5 px-6 py-4 bg-destructive/5 border-b border-destructive/20">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              These actions are permanent and affect the entire platform.
            </p>
          </div>
        </div>
          ...
          {!danger ? (
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium">Remove admin account</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Permanently removes this admin account. Platform data and organizer accounts are unaffected.
                  </p>
                </div>
              </div>
              <Button
